...
_params = {
    ...
    "threads": 60,                          # [int] threads num -th 50
...
def reprocess(subdomain):
    try:
        subdomains.append(subdomain)
    except Exception as e:
        print("Subdomain "+subdomain+" not included")    

def main():
    domain, params = Start.arguments()
    execute(domain, params)

def execute(domain, params):
    # action: scan
    if not params["silent_mode"]:
